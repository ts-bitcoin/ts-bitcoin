...
describe('Script', () => {
    ...
    describe('@fromSafeData', () => {
        ...
        it('should create valid op return output', () => {
            ...
            Script.fromSafeDataArray([Buffer.from('ff', 'hex'), Buffer.from('aa', 'hex')])
                .toString()
                .should.equal('OP_0 OP_RETURN 1 0xff 1 0xaa')
        })
    })
    ...
    describe('#isScriptHashOut', () => {
        ...
        it('should classify these known non-pubKeyHashout as not pubKeyHashout', () => {
            new Script()
                ...
                .should.equal(false)
            new Script()
                .writeString('OP_HASH160 21 0x000000000000000000000000000000000000000000 OP_EQUAL')
                .isScriptHashOut()
                .should.equal(false)
        })
    })
    ...
    describe('#writeNumber', () => {
        it('should write these numbers', () => {
            ...
            new Script().writeNumber(-1).toBuffer().toString('hex').should.equal('4f')
            new Script().writeNumber(-2).toBuffer().toString('hex').should.equal('0182')
        })
    })
    ...
    describe('@writeNumber', () => {
        it('should write these numbers', () => {
            ...
            Script.writeNumber(-1).toBuffer().toString('hex').should.equal('4f')
            Script.writeNumber(-2).toBuffer().toString('hex').should.equal('0182')
        })
    })
    ...
    describe('#setChunkBn', () => {
        it('should set this bn', () => {
            ...
            script.chunks[0].buf
                .toString('hex')
                .should.equal(new Bn(5000000000000000).toBuffer({ endian: 'little' }).toString('hex'))
        })
    })
    ...
    describe('#isOpReturn', () => {
        ...
        it('returns false if the script starts with an opcode that is not OP_RETURN', () => {
            const script = new Script().writeOpCode(OpCode.OP_ADD)
            should(script.isOpReturn()).be.false()
        })

        it('returns false if the script starts with a push data', () => {
            const script = new Script().writeBuffer(Buffer.from("I'm a buffer"))
            should(script.isOpReturn()).be.false()
        })

        it('returns false if the script starts with "OP_FALSE OP_RETURN"', () => {
            const script = new Script().writeOpCode(OpCode.OP_FALSE).writeOpCode(OpCode.OP_RETURN)
            should(script.isOpReturn()).be.false()
        })

        it('returns false if the script starts OP_RETURN and is followed by an OP_FALSE', () => {
            const script = new Script().writeOpCode(OpCode.OP_RETURN).writeOpCode(OpCode.OP_FALSE)
            should(script.isOpReturn()).be.false()
        })
    ...
    describe('#isNonSpendable', () => {
        ...
        it('returns false when starts with OP_FALSE but is followed by opcode that is not OP_RETURN', () => {
            const script = new Script().writeOpCode(OpCode.OP_FALSE).writeOpCode(OpCode.OP_ADD)
            should(script.isNonSpendable()).be.false()
        })

        it('returns false when is only OP_FALSE', () => {
            const script = new Script().writeOpCode(OpCode.OP_FALSE)
            should(script.isNonSpendable()).be.false()
        })
